...
<html>
  <head>
    ...
    <?php
      ...
      $desc = "People you're following on Circlepanda";
  ...
  <body>
    ...
    <section class="house">
      ...
      <div class="postarea_plus">
        <?php
          $fol = "SELECT * FROM follow_friend f JOIN users u ON u.user_id != '$user_id' WHERE f.friends_id = u.user_id and f.user_id = '$user_id'";
            ...
            if ($result->num_rows > 0) {
              while ($user = mysqli_fetch_array($result)) {
                ...
                '. userCovercheck($conn, $user['user_cover_id']) .'
                  <img src="'. $user['profile_pic_id'] .'" class="ico-dp w3-circle">
                  ...
                  <button type="button" class="inner-btn follow-user-btn active">Following</button>
                  </div>
                </a>';
              }
            } else {
              echo '<div class="w3-container nonestat-info w3-center"> You are not following any one at the moment </div>';
            }
          ?>
        </div>
    </section>
