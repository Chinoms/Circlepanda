<?php
  /*
  $regularPost = "SELECT * FROM user_post WHERE user_id='$user_id' ORDER BY post_id DESC LIMIT 1";
...
?>
<?php
  $regularPost = "SELECT * FROM follow_friend f JOIN user_post p ON f.friends_id = p.user_id WHERE f.user_id='$user_id' ORDER BY post_id DESC LIMIT 10";
  $result = mysqli_query($conn, $regularPost);
    while ($post = mysqli_fetch_array($result)) {
      echo '<div class="box">
        <span class="col-sm-12">
          ...
          <div class="post-info">
            <a href="' . BASE_URL . 'friend/'. $convertIdto->idto($conn, $post['user_id'], "user_name") .'" class="name"> '. $post['fullname'] .' <span class="bold"> <i class="fa fa-caret-right"></i> Public </span> </a>
            <time class="float-right"> <i class="material-icons post-option">more_vert</i>
              '. $option->postoption($conn, $post['post_id'], "user_post") .'
            </time>
            <time class="post-time"> '. $time->time_elapsed_string($post['date']) .' . <i class="fa fa-globe"></i> . Public</time>
          </div>
        </span>
        '. $image->get_web_path(imagOrNot($conn, $post['post_id'])) .'
        <div class="w3-container top-space">
          <p class="status pad-top">'. $post['status'] .'</p>
        </div>
        <div class="w3-container">
          <a href="#like" class="icon-below w3-circle left-float space ' . $likes->checklike($conn, $post['post_id'], $user_id, "user_post", "like") .' like" data-postid="' . $post['post_id'] . '">
            <i class="material-icons md-16">thumb_up</i>
          </a>
