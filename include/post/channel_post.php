    ...
    while ($post = mysqli_fetch_array($result)) {
      echo '<div class="box">
        <span class="col-sm-12">
          <img src="' . idto($conn, $post['user_id'], "profile_pic_id") .'" class="dp w3-circle" alt="Display Photo">
          <div class="post-info">
            <a href="' . BASE_URL . 'friend/'. idto($conn, $post['user_id'], "user_name") .'" class="name" style="color:#888;"> '. idto($conn, $post['user_id'], "fullname") .' <span class="bold" style="color:'. $post['channel_color'] .'"> <i class="fa fa-caret-right"></i> '. $post['channel_name'] .' </span> </a>
            <time class="float-right"> <i class="material-icons post-option">more_vert</i>
              '. $option->postoption($conn, $post['post_id'], $tbname) .'
            </time>
