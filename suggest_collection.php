<?php
  ...
  if(!isset($_SESSION['user_id'])) {
      $_SESSION['prevented_page'] = BASE_URL . "suggest_collection";
...
<html>
  <head>
    <meta charset="utf-8">
    <title> Suggested collections </title>
    <?php
      include_once 'metas/seo.php';
      $desc = "Let Circlepanda find you the best Collection";
  ...
  <body>
    ...
    <section class="house">
      ...
      <div class="postarea_plus">
        ...
        # $fetch = "SELECT * FROM collection WHERE user_id != $user_id ORDER BY Collection_id DESC LIMIT 30";
        $fetch = "SELECT * FROM collection c JOIN	join_collection j ON j.collection_id = c.Collection_id WHERE j.user_id!='$user_id' ORDER BY c.Collection_id DESC LIMIT 30";
        $result = mysqli_query($conn, $fetch);
          while ($row = mysqli_fetch_array($result)) {
            echo '<a href="'. BASE_URL .'collection_home/'. $row['Collection_id'] .'" class="display-box" style="color: #fff; background-color:'.$row['Collection_color'].'">
    ...
    <script type="text/javascript">
        ...
        window.onbeforeunload = function()        //When the user leaves the page(closes the window/tab, clicks a link)...
        {
            ...
            var where = "suggested collection";
            var url = "module/analytic.php?time="+timeSpent+"&where="+where;        //Send the time on the page to a php script of your choosing.
